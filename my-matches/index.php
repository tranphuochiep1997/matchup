...
FROM matches m, detail d where m.match_id = d.match_id and m.status >=1 and d.player_id = '$player_id'";

$responseXML = new DOMDocument("1.0");
$responseXML->appendChild($responseXML->createProcessingInstruction('xml-stylesheet', 'type="text/xsl" href="xsl.php"'));
...
if($result = mysqli_query($link, $sql)){
  while($row = mysqli_fetch_array($result)) {
    ...
    $xml_match_id = $responseXML->createElement("matchId",$row["match_id"]);
